...
일반적인 ECMAScript 객체 중 하나 인 함수에 대해 알아보자. 특히 다양한 유형의 함수를 살펴보면서 각 유형이 컨텍스트의 Variables Object(VO)에 미치는 영향과 각 함수의 Scope Chain에 포함된 내용을 알아보자.

우리는 다음과 같은 질문을 받을 수 있다. **다음과 같이 생성 된 함수간에 차이가 있나요? (있는 경우 무엇이 있습니까?)**
...
(function () {...})(); // 다음과 같은 함수는 왜 괄호로 묶어주어야하나?
```
...
> [Variable object]()과 [Scope chain](./4-ScopeChain.md)이 선행되어야 한다.

## 함수의 종류

`ECMAScript`에는 세 종류의 함수 타입이 있고, 각각은 고유한 특징을 가진다.

### Function Declaration(함수 선언식)

함수 선언식(줄여서 FD)은 아래와 같은 특징을 가진다.

- 반드시 이름을 가진다.
- 해당 코드 위치에 존재: 프로그램 레벨이나 다른 함수의 body안에 직접 위치한다.
- 컨텍스트 *진입 시점에 생성*된다.
- *변수 객체(VO)에 영향을 준다.*
...
가장 중요한 특징은 **변수 객체(VO)에 영향을 미친다는 것이다(이것들은 컨텍스트의 변수 객체(VO)에 저장).**

위의 의미는 코드 실행 단계에서 이미 사용 가능한 코드 실행 단계(컨텍스트 단계 진입시 FD는 VO에 저장되어 있음)의 두 번째 중요한 포인트로 정의된다는 것이다.
...
그렇다면 위의 예제에서 `Global`의 `VO`에 `foo`가  들어있을 것이다(흔히 *Hoisting(호이스팅)* 라 불린다).

코드 내 함수를 정의하는 위치는 중요하다.
...
function globalFD() {
	...
	// 2) 또는 다른 함수의 body 내 선언.
	function innerFD() {}
}
```

함수를 선언할 수 있는 위치는 결국 두 군데이다.

### Function Expression(함수 표현식)

함수 표현식(줄여서 FE)은 아래와 같은 특징을 가진다.

- *표현식 위치*에만 정의할 수 있다.
- 선택적으로 이름을 가질 수 있다(없을 수도 있음).
- 함수 표현은 변수 객체에 영향을 주지 않는다.
- *코드 실행 시점에 만들어진다.*

이 유형의 함수의 주요 특징은 **코드에서 항상 표현식 위치에 있다는 것**이다.
...
할당이 끝나면 `foo`를 호출할 수 있다 - `foo()`. 선택적으로 이름을 줄 수 있다.
...
여기에서 주목해야 할 것은 함수 내부에서 `_foo` 라는 이름을 사용할 수 있을 뿐만 아니라(외부는 사용불가), FE의 바깥에서는 식별자 `foo`으로 접근할 수 있다는 사실이다.

`FE`를 식별자에 할당하면 `FD`와 구분하기 어려워진다. 하지만 `FE`가 **항상 표현식 위치에** 있다는 사실을 알고 있다면, 둘을 쉽게 구분할 수 있다.
...
`FE`를 변수에 할당하면, 함수는 메모리에 계속 존재한다. 따라서 나중에 변수명으로 접근할 수 있다(알고 있듯이 변수가 변수 객체(VO)에 영향을 주기 때문).
...
**그 답은 바로 표현식 구문이 가지는 제약 때문이다.**

표준에 따라 표현식 구문은 여는 중괄호, `{` 로 시작할 수 없다. 블럭과 구분할 수 없기 때문이다. 그리고 함수 선언과 구분하기 힘들기 때문에 함수 키워드로 시작해서도 안된다. 

다시 말해서, 즉시 실행 함수(function 키워드로 시작하는)를 만들기 위해서 아래와 같이 함수 선언식을 작성한다면,
...
지금까지 설명한 내용을 가지고, 어떻게 파서에게 함수를 생성과 동시에 실행할 수 있을까?
...
괄호를 감싸는 방법 외에 함수를 FE타입으로 변경할 수 있는 다른 방법이 있다.
...
그룹 괄호는 가장 광범위하고 우아한 방법이다.

그런데 그룹화 연산자는 호출 괄호없이 호출 설명을 포함하여 함수 설명을 둘러쌀 수 있다. 즉 아래의 두 표현식 모두 올바른 FE다.
...
표준에 비춰볼 때 이 구조는 문제가 있다. 코드 블럭 안에 함수 선언식(`FD`)을 둘 수 없기 때문이다(지금은 `if`와 `else`가 `FD`를 가지고 있음). 위에서 이야기 했듯이, `FD`는 프로그램 레벨이나 다른 함수의 body안에 직접 위치해야 한다.
